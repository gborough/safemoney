...
module Make (Qv : Qv_intf.S) = struct
  exception CurrencyTypeMismatch of string [@@deriving sexp, yojson]
  ...
  let check_sym t1 t2 = if String.compare t1.symbol t2.symbol = 0 then true else false
  ...
  let ( + ) t1 t2 =
    if check_sym t1 t2
    then { t1 with value = Qv.S.add t1.value t2.value }
    else raise (CurrencyTypeMismatch "cannot operate on two different currency types")
  ;;

  let ( - ) t1 t2 =
    if check_sym t1 t2
    then { t1 with value = Qv.S.sub t1.value t2.value }
    else raise (CurrencyTypeMismatch "cannot operate on two different currency types")
  ;;
