...
module Make (Qv: Qv_intf.S) = struct
  ...
  let make_qv (symbol, qv) = { symbol = symbol; value = qv; }
  ...
  let to_showable_json x = Yojson.Safe.to_string @@ showable_to_yojson { symbol_ = x.symbol; value_ = Qv.S.to_str x.value }
end
