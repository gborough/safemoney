...
module Separator : sig
  ...
  let make_exn (c1, c2) =
    if Char.is_digit c1 || Char.is_whitespace c1
    then failwith "C1 separator type not allowed"
    else (
      match c2 with
      | Some c2 ->
        if Char.compare c1 c2 = 0
        then failwith "Separator must be different as per convention"
        else if Char.is_digit c2 || Char.is_whitespace c2
        then failwith "C2 separator type not allowed"
...
let mark_thousands ~v ~sep =
  let k = 1000 in
  if Int.compare v k < 0
  then Int.to_string v
  else (
    ...
    let aux ~sep ~i =
      ...
      | quo, rem ->
        if Int.compare rem 10 < 0
        then Some (sep ^ "0" ^ "0" ^ Int.to_string rem, quo)
        else if Int.compare rem 100 < 0
        then Some (sep ^ "0" ^ Int.to_string rem, quo)
...
let q_to_decimal ~printing_conf ~qv =
  ...
  | Truncate ->
    ...
    let f_part v =
      Option.value (List.nth (String.split (Float.to_string v) ~on:'.') 1) ~default:""
    in
    ...
    (match fpair with
     ...
     | i, f ->
       ...
       if len > 0
       then sign ^ ipart ^ sep1 ^ dig ^ aux "" len "0"
  ...
  let parser =
    lift3
      (fun sign ipart fpart ->
        Q.to_string @@ Q.of_float @@ Float.of_string (sign ^ ipart ^ sep1 ^ fpart))
      (peek_char
       >>= function
       | Some '-' -> advance 1 >>| fun () -> "-"
       | Some '+' -> advance 1 >>| fun () -> ""
       ...
       | Some sep2 ->
         lift2
           ...
           (count 3 (satisfy Char.is_digit)
            ...
            >>| String.of_char_list)
           (many @@ (char (Char.of_string sep2) *> count 3 (satisfy Char.is_digit))
            >>| List.concat
            >>| String.of_char_list))
