...
module Make (Qv : Qv_intf.S) = struct
  ...
  let show_xchg t =
    Printf.printf "Exchange rate from %s to %s: %s" t.src t.dst (Qv.S.to_str t.value)
  ;;

  let ( **> ) t1 t2 =
    if String.compare t1.dst t2.src = 0
    then { src = t1.src; dst = t2.dst; value = Qv.S.mul t1.value t2.value }
    else
      raise
        (IntermediaryMismatch
           "Intermediary currency symbol must be equal in order to compose exchange rate")
  ;;
  ...
  let to_showable_json x =
    ...
    @@ showable_to_yojson { src_ = x.src; dst_ = x.dst; value_ = Qv.S.to_str x.value }
  ;;
end
