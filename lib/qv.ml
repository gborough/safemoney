...
module S = struct
       ...
       | Q.INF | Q.MINF | Q.UNDEF ->
         raise
           (NonsensicalQuotientValue
              "nonsensical quotient values are not suitable for currency operation")
       | Q.NZERO ->
         if Z.compare q.den Z.zero = 0
         then raise (DenominatorZero "denominator must not be zero")
         else q
       | Q.ZERO -> Q.zero)
  ...
  let to_str x =
    if Z.compare (den_of_q x) Z.one = 0 then Q.to_string x ^ "/1" else Q.to_string x
  ;;
