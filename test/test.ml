...
let%expect_test "qv_ops" =
  ...
  let r =
    abs
      (neg (( * ) ~t:(qv1 + qv2 - qv2) ~value:(make_q "3/1") / make_q "3/1"))
  in
  qv_to_str r.value |> print_endline ;
  [%expect {|3/1|}]
...
let () =
  ...
  try
    ...
    let _r =
      abs
        (neg
           (( * ) ~t:(qv1 + qv2 - qv2) ~value:(make_q "3/1") / make_q "3/1") )
    in
    Printf.printf "%s" ""
  with
...
let%expect_test "dv_ops_with_scale" =
  ...
  zv_to_str r.value |> print_endline ;
  [%expect {|2000|}]
...
let%expect_test "xchg_ops" =
  ...
  qv_to_str gbp_to_nzd.value |> print_endline ;
  [%expect {|
    GBP
    NZD
    14514592/6759003 |}]
...
let%expect_test "predefined" =
  ...
  qv_to_str rv.value |> print_endline ;
  zv_to_str dv.value |> print_endline ;
  [%expect {|
    3/1
    2000|}]
...
let%expect_test "qv_to_decimal_up" =
  ...
  in
  q_to_decimal ~printing_conf ~qv:(make_q "-1234567/7") |> print_endline ;
  [%expect {|-176,366.0|}]
...
let%expect_test "unsafe_float_to_discrete" =
  ...
  [%expect
    {|{"discrete_scale":"{\"symbol\":\"GBP\",\"unit\":\"Penny\",\"value\":\"100/1\"}","discrete_value":"{\"value\":\"123\"}"}|}]
